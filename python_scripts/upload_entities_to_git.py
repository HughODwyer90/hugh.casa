...
from secret_manager import SecretsManager  # Import the SecretsManager class from git_uploader import GitHubUploader  # Import the GitHubUploader class from html_generator import HTMLGenerator  # Import the HTMLGenerator class


try: # Load secrets secrets = SecretsManager()
...
if response.status_code == 200:
    ...
    # Automatically exclude input_text helpers with mode: password and anything containing "GPS"
    filtered_entities = [
        entity for entity in entities 
        if not (entity["entity_id"].startswith("input_text.") and 
